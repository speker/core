...
class Redis(object):
    ...
    RESPONSE_CALLBACKS = dict_merge(
        ...
        string_keys_to_dict('BGREWRITEAOF BGSAVE', lambda r: True),
        {
            ...
            'RANDOMKEY': lambda r: r and str(r) or None,
            'SCAN': parse_scan,
    ...
    def sismember(self, name, value):
        "Return a boolean indicating if ``value`` is a user of set ``name``"
    ...
    def spop(self, name, count=None):
        "Remove and return a random user of set ``name``"
        args = (count is not None) and [count] or []
        return self.execute_command('SPOP', name, *args)

    def srandmember(self, name, number=None):
        """
        If ``number`` is None, returns a random user of set ``name``.
    ...
    def bzpopmax(self, keys, timeout=0):
        ...
        then block for ``timeout`` seconds, or until a user gets added
    ...
    def zrem(self, name, *values):
        "Remove user ``values`` from sorted set ``name``"
    ...
    def georadiusbymember(self, name, member, radius, unit=None,
        ...
        and latitude value, it takes the name of a user already existing
